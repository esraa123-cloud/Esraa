...
const formatSwap = (swapDoc, currentUserId) => {
  ...
  const isProposer = swap.proposer && String(swap.proposer._id || swap.proposer) === String(currentUserId);
  const isReceiver = swap.receiver && String(swap.receiver._id || swap.receiver) === String(currentUserId);

  const proposerName = isProposer ? 'أنت' : (swap.proposer && swap.proposer.fname ? `${swap.proposer.fname} ${swap.proposer.lname}` : 'مستخدم');
  const receiverName = isReceiver ? 'أنت' : (swap.receiver && swap.receiver.fname ? `${swap.receiver.fname} ${swap.receiver.lname}` : 'مستخدم');
...
const setStatus = (allowedFrom, newStatus) => async (req, res, next) => {
  try {
    ...
    // Accept and Reject can only be performed by the receiver of the request.
    ...
    res.json({ success: true, message: 'تم تحديث حالة الطلب بنجاح', swap: formatSwap(populated, req.user._id) });
  } catch (err) {
    next(err);
  }
};
