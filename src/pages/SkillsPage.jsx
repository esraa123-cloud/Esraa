...
export const SkillsPage = ({ setActiveTab }) => {
  const { skills, searchQuery, setSearchQuery, selectedCategory, setSelectedCategory, addSkill, loadingSkills } = useSwap();
  ...
  const handleAddSkillSubmit = async (e) => {
    ...
    if (!isLoggedIn) {
      alert('يرجى تسجيل الدخول أولاً لإضافة مهارة جديدة');
      setActiveTab('login');
      return;
    }
    ...
    await addSkill({
      ...
      icon: newIcon || categoryIcons[newCategory] || '💡',
      wants: newWants
    });
  ...
  const handleDirectChat = async (ownerName, ownerId) => {
    if (!isLoggedIn) {
      alert('يرجى تسجيل الدخول لمراسلة الأعضاء');
      setActiveTab('login');
      return;
    }
    await startChatWithUser(ownerName, null, ownerId);
    setActiveTab('messages');
  };

  const handleSwapClick = (skill) => {
    if (!isLoggedIn) {
      alert('يرجى تسجيل الدخول أولاً لطلب تبادل مهارة');
      setActiveTab('login');
      return;
    }
          ...
          <div className="modal-card">
            ...
            <form onSubmit={handleAddSkillSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setNewIcon(categoryIcons[e.target.value] || '💡');
                  }}
                >
              ...
              <button type="submit" className="btn-submit" style={{ marginTop: '0.5rem' }} disabled={isSubmitting}>
                {isSubmitting ? 'جاري النشر...' : 'نشر المهارة الان'}
              </button>
            </form>
          </div>
        </div>
